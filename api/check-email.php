...
/**
 * Email IMAP Checker for Verification Codes and User Messages
 * Actively polls Gmail inbox for replies to verification emails and direct user messages
 * Run this via cron job or manually to check for new emails
 */
...
function logMessage($message)
{
    ...
    error_log("Email Checker: $message");
}
...
try {
    ...
    foreach ($emails as $email_number) {
        ...
        // This is a regular user message (no verification code found)
        else {
            logMessage("No verification code found - treating as regular user message");
            ...
            $query = "
                ...
                FROM users
                WHERE email = '$sender_email_escaped'
            ";
            ...
            $subject_escaped = mysqli_real_escape_string($GLOBALS['conn'], $subject);
            $body_escaped = mysqli_real_escape_string($GLOBALS['conn'], $body);

            $user_id = $user ? $user['user_id'] : 'NULL';
            $user_name = $user ? mysqli_real_escape_string($GLOBALS['conn'], $user['user_name']) : 'Unknown User';
            ...
            if (executeQuery($insert_query)) {
                $new_messages_count++;
                logMessage("New user message saved from: $sender_email ($user_name)");

                // Send auto-reply to acknowledge receipt
                require_once '../config/mail.php';
                $ack_subject = "Message Received - " . $subject;
                $ack_body = "
                    <h3>Thank you for your message!</h3>
                    <p>We have received your message with subject: <strong>" . htmlspecialchars($subject) . "</strong></p>
                    <p>Our support team will review your message and get back to you shortly.</p>
                    <br>
                    <p>Best regards,<br>BookStack Support Team</p>
                ";

                sendEmail($sender_email, $ack_subject, $ack_body);
                logMessage("Auto-reply sent to: $sender_email");
            } else {
                logMessage("Failed to save message from: $sender_email");
            }
    ...
    logMessage("Email processing complete. Processed: $processed_count, Updated: $updated_count, New Messages: $new_messages_count");
    ...
    if (!$is_cli) {
        ...
        echo "Emails processed: $processed_count checked, $updated_count verification updates, $new_messages_count new messages\n";
    }
} catch (Exception $e) {
